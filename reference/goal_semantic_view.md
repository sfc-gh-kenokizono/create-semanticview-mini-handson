...
| テーブル | 主キー | 説明 | 備考 |
|---------|--------|------|------|
| **PRODUCTS** | PRODUCT_ID | 製品マスタ（商品ID、商品名、カテゴリ） | テーブル定義で設定済み |
| **SALES** | なし | 売上データ（日付、地域、商品ID、数量、金額） | PRODUCTSを参照 |
| **MARKETING_CAMPAIGN_METRICS** | CATEGORY | マーケティング指標（インプレッション、クリック） | テーブル定義で設定済み |
| **SOCIAL_MEDIA** | なし | SNSデータ（プラットフォーム、インフルエンサー、メンション） | MARKETINGを参照 |
