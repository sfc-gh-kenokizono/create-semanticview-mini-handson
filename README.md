...
各クエリには**質問（自然言語）**と**SQLクエリ**の両方を入力します。
「追加（Shift + Enter）」で複数のクエリを追加できます。
...
| 項目 | 入力内容 |
|------|---------|
| 質問 | `SNSプラットフォーム別のメンション数とクリック数を比較して` |
...
> 💡 **ポイント**: 質問とSQLのペアを入力することで、AIが「この質問にはこのSQLが対応する」というパターンを学習します。
> また、SQLのJOIN条件からリレーションシップも自動検出されます。
